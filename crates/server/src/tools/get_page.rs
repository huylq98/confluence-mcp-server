...
pub fn format(
    ...
    header.push_str(&format!(
        "Space: {space_name} ({space_key}) | Version: {version}\n"
    ));
    if !labels.is_empty() {
        header.push_str(&format!("Labels: {labels}\n"));
    }
    if !ancestors.is_empty() {
        header.push_str(&format!("Path: {ancestors} → {title}\n"));
    }
    if !url.is_empty() {
    ...
    let raw_body = page
        .pointer(&format!("/body/{body_format}/value"))
        .and_then(Value::as_str)
        .unwrap_or("");
    let body = if body_format == "view" {
        strip_html(raw_body)
    } else {
        raw_body.to_string()
    };
