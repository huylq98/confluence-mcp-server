...
pub fn analyze(history: &[HistoryRow], now_ts: i64) -> Vec<Tip> {
    ...
    if let Some(t) = repeated_page_fetch(&recent) {
        tips.push(t);
    }
    if let Some(t) = oversized_output(&recent) {
        tips.push(t);
    }
    if let Some(t) = high_error_rate(&recent) {
        tips.push(t);
    }
    if let Some(t) = frequent_403s(&recent) {
        tips.push(t);
    }
    tips
}
...
fn oversized_output(rows: &[&HistoryRow]) -> Option<Tip> {
    let n = rows
        .iter()
        .filter(|r| r.tokens_est > OVERSIZED_TOKENS)
        .count();
    if n == 0 {
        return None;
    }
...
fn high_error_rate(rows: &[&HistoryRow]) -> Option<Tip> {
    if rows.len() < MIN_CALLS_FOR_ERROR_RATE {
        return None;
    }
...
mod tests {
    ...
    fn high_error_rate_fires_above_ten_percent() {
        ...
        for i in 0..8 {
            rows.push(row(now - i, "get_page", "ok", 50, json!({"page_id":"1"})));
        }
        for i in 0..2 {
            rows.push(row(
                now - 20 - i,
    ...
    fn frequent_403s_fires_on_same_page() {
        let now = 1_700_000_000;
        let rows: Vec<HistoryRow> = (0..3)
            .map(|i| row(now - i, "get_page", "403", 50, json!({"page_id":"99"})))
            .collect();
        let tips = analyze(&rows, now);
    ...
    fn high_error_rate_silent_on_fresh_install() {
        ...
        let tips = analyze(&rows, now);
        assert!(
            !tips.iter().any(|t| t.id == "high_error_rate"),
